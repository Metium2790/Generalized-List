...
namespace Generalized_List
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("3-Add to Down List");
            Console.WriteLine("4-Find depth of the list");
            Console.WriteLine("5-Print the list");
            Console.WriteLine("6-Find all amounts of the nodes");
            Console.WriteLine("7-Find a data and delete them from the list");
            Console.WriteLine("8-Make a double list and add all the list item to it");
            Console.WriteLine("9-Compare 2 list to each other");
            ...
            while (true)
            {
                ...
                switch (CMD)
                {
                    ...
                    case "3":
                        ...
                        Console.WriteLine("Please give us the number of your node inside the list which you want to add a down list to it(first=0)(cancel=-1)");
                        int number = Int32.Parse(Console.ReadLine());

                        Console.WriteLine("Your Data inside the new node: ");
                        Node newnode = new Node(Console.ReadLine());

                        lists[listnum - 1].Add_dlink(lists[listnum - 1].FindNode(number), newnode);
                        break;
                    case "2":
                        Console.WriteLine("which list you want to add choose?");
                        listnum = Int32.Parse(Console.ReadLine());
                        Console.WriteLine("Your Data inside the new node: ");
                        Node newnode2 = new Node(Console.ReadLine());
                        lists[listnum-1].Add(newnode2);
